...
// Manual mappings for colleges that don't exactly match
...
function findCollegeInfo(collegeName) {
  ...
  // Check exact match
  if (collegeMapping[normalizedName]) {
    return collegeMapping[normalizedName];
  }
  
  // Check manual mappings
  if (manualMappings[normalizedName]) {
    return manualMappings[normalizedName];
  }
  
  // Try fuzzy matching - check if college name contains key phrases
